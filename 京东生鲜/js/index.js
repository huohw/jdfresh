...
function autoMove() {
	...
	wrapper.style.left = -step * 720 + 'px';
	paginationFocus();
}
...
/* 选项卡部分 */
$headItem = $('.tab_head_item'),
$bodyItem = $('.tab_body_item');

$headItem.on('click',function(){
	// this 就是当前点击的这个原生元素
	// 给当前点击的这个元素加上 active类名； 让其他兄弟元素移除active类名
	$(this).addClass('active').siblings().removeClass('active');
	let n = $(this).index();// n就是点击的这个li的索引

	// $('div')[n] 当前点击的哪个li对应的div(原生DOM) 给他添加active类名 让他的兄弟们移除active类名
	// $($('div')[n]).addClass('active').siblings().removeClass('active');
	// $('div').eq(n) 获取的是索引n对应的 JQ对象；
	$bodyItem.eq(n).addClass('active').siblings().removeClass('active');
  });
